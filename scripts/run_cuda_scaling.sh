...
CSV_FILE="${RESULTS_DIR}/cuda_scaling.csv"
LOG_FILE="${RESULTS_DIR}/cuda_scaling.log"
...
for chains in "${chains_list[@]}"; do
  for block_size in "${block_sizes[@]}"; do
    echo "[run] SA CUDA chains=${chains} block_size=${block_size}"
    "${TSP_SA}" \
      --input "${runtime_input}" \
      --parallel cuda \
      --chains "${chains}" \
      ...
      --seed "${seed}" \
      --init nn \
      --csv-only >> "${CSV_FILE}" 2>> "${LOG_FILE}"

    echo "[run] QLSA CUDA chains=${chains} block_size=${block_size}"
    "${TSP_SA}" \
      --input "${runtime_input}" \
      --qlsa \
      --parallel cuda \
...
echo "CUDA scaling CSV written to ${CSV_FILE}."
echo "CUDA scaling stderr log written to ${LOG_FILE}."
